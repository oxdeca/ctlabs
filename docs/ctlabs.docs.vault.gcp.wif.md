...
3. **Define a role, with `client_id` set to the GCP audience string.** Vault's
   ...
   [...] does not match the expected audience`. **This bug hit us twice**
   (2026-09-23 on the first host, then again 2026-09-24 setting up a second
   host reusing the same pool with a different `provider-id`) - both times
   because `client_id` was easy to skip on a first pass and easy to forget to
   circle back to. `vault_oidc_setup.py` now makes this a **hard requirement**
   instead of an optional flag with a warning - it refuses to run without
   either `--client-id` or `--project-number` (see below).

   The audience string is fully computable **before the GCP pool/provider
   even exist** - project number is a property of the project itself (not
   generated by pool/provider creation), and pool/provider names are just
   names you pick. So there's no real chicken-and-egg problem, only an
   easy-to-skip step:
...
```
   ...
   Or let `vault_oidc_setup.py` build the audience itself - pass
   `--project-number` (+ `--pool-id`/`--provider-id` if not using the
   defaults) instead of constructing the string by hand:

```bash
python3 vault_oidc_setup.py --issuer https://vdb1.ctlabs.internal:8200 \
    --project-number "${PROJECT_NUMBER}"
```

   **When reusing the same pool for a second host with a different
   `provider-id`**: pass the matching `--provider-id` here too - the audience
   string embeds the provider-id, so it's different per host even within the
   same pool. Copy-pasting a previous host's full `--client-id` string (or
   its printed `gcp_wif_setup.py` command) is exactly what caused the
   2026-09-24 recurrence.
