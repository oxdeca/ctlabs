...
# Kill running SConfig / cmd window if it launched during first logon
Get-Process -Name "sconfig", "cmd" -ErrorAction SilentlyContinue | 
    Where-Object { $_.CommandLine -like "*sconfig*" -or $_.MainWindowTitle -like "*SConfig*" } | 
...
if ($info.LastTaskResult -ne 0) {$detail = if (Test-Path $logPath) { Get-Content$logPath -Raw } else { "(no log at $logPath)" }
...
Get-Volume | Where-Object { $_.DriveType -eq "CD-ROM" -and $_.DriveLetter } \vert{} ForEach-Object {$script = "$($_.DriveLetter):\ctlabs_net_setup.ps1"
    if (Test-Path $script) {
        & $script
    }
}
